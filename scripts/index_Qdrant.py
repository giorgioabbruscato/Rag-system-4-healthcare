...
def _ensure_collections_populated():
    ...
    try:
        ...
        if "cases" in collection_names:
            logger.info(
                "[IndexQdrant] Collection 'cases' exists, checking if populated..."
            )
            try:
...
def _create_and_index_all():
    ...
    vector_config = [
        VectorConfig(name=settings.vector_name, dimensions=settings.embedding_dim)
    ]
    ...
    try:
        ...
        logger.warning(
            "Collection 'cases' could not be deleted or does not exist", error=str(e)
        )
    ...
    try:
        ...
        logger.warning(
            "Collection 'guidelines' could not be deleted or does not exist",
            error=str(e),
        )
...
def _index_cases():
    ...
    for i in range(len(docs_text)):
        ...
        chunks.append(
            Chunk(id=doc_uuid, text=docs_text[i], embeddings=[emb], metadata=metadata)
        )
...
def _index_guidelines():
    ...
    print(
        f"[IndexQdrant] ✓ Indexed {len(docs_text)} guideline chunks from {len(set(m['source'] for m in docs_metadata))} files."
    )
